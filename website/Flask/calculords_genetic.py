...
pla = [] #cards that can be played
math_orig = [] #cards that can be manipulated to make play cards
gens = 600
math = []
    ...
    for name in output:
        if output[name].isdigit():
            ...
            if name[0:4] == "play":
                ...
                math.append(num)
    print(play, math)
    results = genetic_algo(play, math)
    
    for i in range(0, len(results[3])):
        results[3][i] = str(i+1) +". "+results[3][i]
    
    for i in range(0, len(results[2])):
        results[2][i] = str(i+1) +". "+results[2][i]
    
    return render_template("index.html", steps = results[3], formula = results[2])
